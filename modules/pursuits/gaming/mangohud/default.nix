{self, ...}: {
  flake.modules.nixos.mangohud = {pkgs, ...}: {
    imports = [self.modules.nixos.steam-options];

    gaming.steam.extraEnv = {
      MANGOHUD = "1";
  ...
  in {
    imports = [self.modules.homeManager.mangohud-options];
    ...
    config = mkIf pkgs.stdenv.hostPlatform.isLinux {
      ...
      xdg.configFile."MangoHud/MangoHud.conf".text = ''
        ...
        ${optionalString (config.internal.gaming.mangohud.pciDevice != null) ''
          pci_dev=${config.internal.gaming.mangohud.pciDevice}
        ''}
        ...
        ${optionalString (config.internal.gaming.mangohud.coolantSensor != null) ''
          custom_text=CLT
          exec=awk '{printf "%.0f°C", $1/1000}' ${config.internal.gaming.mangohud.coolantSensor}
        ''}
        custom_text=
        ram
        procmem
